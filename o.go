...
func (conn *DmConnection) setTrxFinish(status int32) {
	switch status & Dm_build_388 {
	case Dm_build_385, Dm_build_386, Dm_build_387:
		conn.trxFinish = true
	default:
...
func (dc *DmConnection) executeInner(query string, execType int16) (interface{}, error) {
	...
	if execType == Dm_build_352 && dc.dmConnector.enRsCache {
...
func (dc *DmConnection) query(query string, args []driver.Value) (*DmRows, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, Dm_build_352)
		if err != nil {
			return nil, err
		}
